...
func (d *deployer) copyKubeConfig(master *clusterv1.Machine) error {
	var config string
	var err error
	eb := util.NewExponentialBackoff(2 * time.Minute, 10 * time.Second)
...
func (d *deployer) waitForApiserver(master string, timeout time.Duration) error {
	...
	if err != nil {
		glog.Errorf("Error waiting for apiserver: %s", err)
		return err
	}
	return nil
}
...
func (d *deployer) waitForServiceAccount(timeout time.Duration) error {
	...
	if err != nil {
		glog.Errorf("Error waiting for service account: %s", err)
		return err
	}
	return nil
}
