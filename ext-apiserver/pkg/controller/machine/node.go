...
func (c *MachineControllerImpl) link(node *corev1.Node) {
...
func (c *MachineControllerImpl) unlink(node *corev1.Node) {
	if val, ok := node.ObjectMeta.Annotations["machine"]; ok {
		machine, err := c.machineClient.Get(val, metav1.GetOptions{})
		if err != nil {
			glog.Errorf("Error getting machine %v: %v\n", val, err)
		...
		c.linkedNodes[node.ObjectMeta.Name] = false

		// Even if the update fails below, either machine object is going away pretty
		// soon or it will be linking with another node object.
		if _, err := c.machineClient.UpdateStatus(machine); err != nil {
